...
# full_data_ind <- which(colSums(!is.na(act)) > 672)
...
for (model_ind in 9:9){
  ...
  working_df <- data.frame(block = sleep_block_vec,
                         ...
                         pov = tran_pov_vec2,
                         pov_cont = id$poverty[full_data_ind],
                         age = tran_age_vec2,
                         age_cont = id$age[full_data_ind],
                         race = id$race[full_data_ind],
                         bmi_cont = id$BMI[full_data_ind],
...
ggplot(sleep_df, aes(x=clust, y=block)) + 
  geom_boxplot()

ggplot(sleep_df, aes(x=clust, y=onset)) + 
  geom_boxplot()

ggplot(sleep_df, aes(x=clust, y=eff)) + 
  geom_boxplot()

ggplot(sleep_df, aes(x=clust, y=totalSleep)) + 
  geom_boxplot()
...
ggplot(sleep_df, aes(x=houseFS, y=block)) + 
  geom_boxplot()

ggplot(sleep_df, aes(x=houseFS, y=onset)) + 
  geom_boxplot()

ggplot(sleep_df, aes(x=houseFS, y=eff)) + 
  geom_boxplot()

ggplot(sleep_df, aes(x=houseFS, y=totalSleep)) + 
  geom_boxplot()
...
ggplot(sleep_df, aes(x=as.factor(age_cont), y=block)) + 
  geom_boxplot(outlier.shape = NA)
...
km <- kmeans(params_to_save[[7]][1,1,],centers = 8,algorithm="Lloyd",iter.max = 1000)
centers <- sort(km$centers)
km <- kmeans(params_to_save[[7]][1,1,],centers = centers,algorithm="Lloyd",iter.max = 10)

km$centers
act_est_array[[8]][1,1,]


sleep_df <- sleep_df%>% mutate(kgroup = as.factor(km$cluster[full_data_ind]))

ggplot(sleep_df, aes(x=clust, y=age_cont)) + 
  geom_boxplot(outlier.shape = NA)

ggplot(sleep_df, aes(x=kgroup, y=age_cont)) + 
  geom_boxplot(outlier.shape = NA)

ggplot(sleep_df, aes(x=clust, y=bmi_cont)) + 
  geom_boxplot(outlier.shape = NA)

ggplot(sleep_df, aes(x=kgroup, y=bmi_cont)) + 
  geom_boxplot(outlier.shape = NA)
```
