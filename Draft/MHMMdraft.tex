...
would be shifted to the right or left depending on $u_i$. The bottom two plots show the different continuous RE 
distributions selected. The 5 choices for H have been split up into 
two plots to increase readability.

To determine the situations where a MHMM outperforms a standard HMM, we tested three different types of HMMs: (i) standard HMM where each person has their own mean wake activity parameter, (ii) standard HMM where there is a shared mean wake activity parameter, (iii) mixed HMM with 2-8 support points as described by section \ref{Methods}. (ii) is a special case of a MHMM and will be referred to as a MHMM with 1 support point. It should be noted that (i) is only possible if there are many measurements for each person and drastically increases the number of estimated parameters. The number of emission parameters for (ii) and (iii) is 3 + the number of support points, which ranges from 4-12 in this study. Looking at (i), the number of emission parameter is 3 + the number of individuals in the data, which ranges from 1,003 - 5,003
...
Now, we turn to the necessary number of support points needed 
for the NPDE of the RE. This is mainly dependent upon 
the underlying continuous RE distribution and not the sample size. There is a large increase in accuracy when the number of support points is increased from 1 to 2, with diminishing accuracy increases afterwards. Accuracy must be balanced with computational time as increasing the number of support points increases the running time. When H is gamma, two support points suffice. NEED TO EXPAND THIS
...
To get a better understanding of how the method is working we examine the sensitivity and positive predictive value (PPV). 
Figure \ref{NL2} shows the sensitivity (left column) and PPV 
...
that we accurately predict as wake, and wake PPV is the percent 
of predicted wake states that are truly wake. FEW SENTENCE ABOUT IND HMM

When we fit a pooled HMM we see that although the sleep sensitivity is high, 
