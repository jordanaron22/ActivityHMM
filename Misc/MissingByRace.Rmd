...
covariate_df <- data.frame(race = AddVariable("RIDRETH3" ,demo, seqn_freq, seqn_list))
...
race_num.df <- data.frame(race = rep(c(c("Mexican American","Other Hispanic","Non-Hispanic White",
                             "Non-Hispanic Black","Non-Hispanic Asian","Other Race - Including Multi-Racial")),2),
...
ggplot(race_num.df,aes(x= as.factor(race), y = participants, fill = datatype))+
  ...
  xlab("Race") + 
  ggtitle("Number of Participants by Race") + 
  labs(fill = "Data Type") + 
  ylim(0,4000)
...
ggplot(data.frame(race_watch),aes(seq_along(race_watch),race_watch))+
  ...
  scale_x_discrete(name ="Race", 
                    limits=str_wrap(c("Mexican American","Other Hispanic","Non-Hispanic White",
                             "Non-Hispanic Black","Non-Hispanic Asian","Other Race - Including Multi-Racial"),width = 15)) + 
  ylab("Percent") + 
  ggtitle("Percent of Participants with Actiwatch Data by Race") + 
  ylim(0,1)
```
...
ggplot(data.frame(race_nonwear),aes(seq_along(race_nonwear),race_nonwear))+
  ...
  scale_x_discrete(name ="Race", 
                    limits=str_wrap(c("Mexican American","Other Hispanic","Non-Hispanic White",
                             "Non-Hispanic Black","Non-Hispanic Asian","Other Race - Including Multi-Racial"),width = 15)) + 
  ylab("Percent") + 
  ggtitle("Actiwatch Non-Wear by Race") + 
  ylim(0,.25)
```
