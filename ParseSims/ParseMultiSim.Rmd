...
ParseSingleSim <- function(folder, path){
  setwd(path)
  setwd(folder)
...
for (folder_ind in 1:length(folder_list)){
  SingleSim <- ParseSingleSim(folder_list[folder_ind],path)
  MultiSim <- append(MultiSim,list(SingleSim))
}
...
Boxplot1D <- function(Multisim,list_ind,vec_ind){

  num_of_sims <- length(MultiSim)
  final_df <- data.frame()
  
  for (sim_ind in 1:num_of_sims){
    est <- MultiSim[[sim_ind]][[list_ind]][[2]][vec_ind,]
    nRE <- MultiSim[[sim_ind]][[8]][[1]]
  ...
  est <- MultiSim[[1]][[list_ind]][[1]][vec_ind,]
  nRE <- "Truth"
  working_df <- data.frame(estimates = est,
                           numberRE = nRE)
  final_df <- rbind(final_df,working_df)
...
Boxplot1D(MultiSim,1,2)
Boxplot1D(MultiSim,1,1)


Boxplot1D(MultiSim,6,1)
