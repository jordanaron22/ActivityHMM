...
ParseSingleClust <- function(folder, path_size ){
  ...
  vit_array2 <- matrix(0,5,length(file_list))
  vit_array3 <- matrix(0,5,length(file_list))
  ...
  for (file_ind in 1:length(file_list)){
    ...
    clust_ind_true <- params_to_save[[1]][[6]]
    q1_ind <- clust_ind_true < 0
    
    decoded_mat_q1 <- params_to_save[[4]][[2]][,q1_ind]
    mc_q1 <- params_to_save[[4]][[1]][,q1_ind]
    
    vit_array2[1,file_ind] <- (sum(decoded_mat_q1 == 0 & mc_q1 ==0) + sum(decoded_mat_q1 == 1 & mc_q1 ==1))/(prod(dim(decoded_mat_q1)))
    vit_array2[2,file_ind] <- sum(decoded_mat_q1 == 0 & mc_q1 ==0)/sum(mc_q1 ==0)
    vit_array2[3,file_ind] <- sum(decoded_mat_q1 == 1 & mc_q1 ==1)/sum(mc_q1 ==1)
  
    vit_array2[4,file_ind] <- sum(decoded_mat_q1 == 0 & mc_q1 ==0)/sum(decoded_mat_q1 ==0)
    vit_array2[5,file_ind] <- sum(decoded_mat_q1 == 1 & mc_q1 ==1)/sum(decoded_mat_q1 == 1) 
    
    ############
    
    clust_ind_true <- params_to_save[[1]][[6]]
    q2_ind <- clust_ind_true > 0
    
    decoded_mat_q2 <- params_to_save[[4]][[2]][,q2_ind]
    mc_q2 <- params_to_save[[4]][[1]][,q2_ind]
    
    vit_array3[1,file_ind] <- (sum(decoded_mat_q2 == 0 & mc_q2 ==0) + sum(decoded_mat_q2 == 1 & mc_q2 ==1))/(prod(dim(decoded_mat_q2)))
    vit_array3[2,file_ind] <- sum(decoded_mat_q2 == 0 & mc_q2 ==0)/sum(mc_q2 ==0)
    vit_array3[3,file_ind] <- sum(decoded_mat_q2 == 1 & mc_q2 ==1)/sum(mc_q2 ==1)
  
    vit_array3[4,file_ind] <- sum(decoded_mat_q2 == 0 & mc_q2 ==0)/sum(decoded_mat_q2 ==0)
    vit_array3[5,file_ind] <- sum(decoded_mat_q2 == 1 & mc_q2 ==1)/sum(decoded_mat_q2 == 1) 
...
for (dist_ind in 1:dist_num){
  ...
  for (size_ind in 1:size_num){
    for (clust_ind in 1:(clust_num+1)){
      ...
      # vit_med <- apply(MultiSim[[dist_ind]][[size_ind]][[clust_ind]][[13]],1,median,na.rm=T)
...
wsens_df <- vit_df %>% filter(.[[1]] == "Wake Sensitivity")
wsens_df <- wsens_df[,-1]
...
plot_data = nested_loop_paramsteps_data(
    plot_data,
    steps_y_base = -1, steps_y_height = .5, steps_y_shift = 1.5
)
...
bic_df <- bic_df %>%mutate(across(Cluster0:Cluster8, ~ ./Cluster0))
...
p = nested_loop_base_plot(
    plot_data,
    x_name = "", 
    y_name = "Relative BIC compared to Individual HMM",
    colors = viridis::viridis(11)[c(3:10)],
    shapes = c(15,16,16,16,16,16,16,16),
    sizes = c(1.5,1,1,1,1,1,1,1),
    legend_labels = c("Shared HMM","MHMM2","MHMM3",
                      "MHMM4","MHMM5","MHMM6",
                      "MHMM7","MHMM8"),
    connect_spus = T
)
...
plot_data = nested_loop_paramsteps_data(
    ...
    steps_y_base = .94, steps_y_height = .00625, steps_y_shift = .0125
)
...
p + labs(title = "Realtive BIC by Model, Sample Size, and RE Distribution")
...
p1 <- ggplot(data = data.frame(x = c(-7.5,7.5)), aes(x)) +
  ...
  labs(x = "Activity", y="Density", title="Distribution of Activity by Wake-Sleep State",color="Wake-Sleep State") +
...
p2 <- ggplot(data = data.frame(x = c(-8, 8)), aes(x)) +
  scale_y_continuous(breaks = NULL) +
  labs(x = "Random Effect", y="Density", title="Random Effect Distribution (H)") + 
...
setwd("../Plots")
ggsave("REdist.png", arrangeGrob(p1, p2),width = 6,height = 6)
