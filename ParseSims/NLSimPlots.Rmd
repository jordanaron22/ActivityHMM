...
ParseSingleClust <- function(folder, path_size ){
  ...
  for (file_ind in 1:length(file_list)){
    ...
    nap_array[file_ind] <- sum(params_to_save[[7]])
...
#     vit_df,
#     x = "xfiller", steps = c("Distribution"),
...
plot_data = nested_loop_paramsteps_data(
    plot_data,
    steps_y_base = 0.25, steps_y_height = .025, steps_y_shift = .05
)
...
plot_data = nested_loop_paramsteps_data(
    plot_data,
    steps_y_base = 0.45, steps_y_height = .025, steps_y_shift = .05
)
...
# setwd("../Plots")
# ggsave("NestedLoopAcc.png",width = 6,height = 5)
```
...
# tran_resid_df <- tran_resid_df[,-c(3,4)]
...
plot_data = nested_loop_paramsteps_data(
    plot_data,
    steps_y_base = -.25, steps_y_height = .25, steps_y_shift = .25
)
...
#     for (clust_ind in 1:(clust_num+1)){
#       nRE <- MultiSim[[dist_ind]][[size_ind]][[clust_ind]][[8]]
#       nap_med <- apply(MultiSim[[dist_ind]][[size_ind]][[clust_ind]][[12]],1,median)
#       nobs <- MultiSim[[dist_ind]][[size_ind]][[clust_ind]][[10]][[3]][1]
#       lobs <- MultiSim[[dist_ind]][[size_ind]][[clust_ind]][[10]][[3]][2]
#       dist <- MultiSim[[dist_ind]][[size_ind]][[clust_ind]][[10]][[4]]
#       
#       working_df <- data.frame(estimates = nap_med,
#                              numberRE = nRE,
#                              numObs = nobs,
#                              lenObs = lobs,
#                              dist = dist)
...
# library(looplot)
# 
# nap_df <- final_df %>% pivot_wider(names_from = numberRE, values_from = estimates,names_prefix = "Cluster") 
# 
# nap_df <- nap_df%>% mutate(xfiller = 1) 
# 
# colnames(nap_df)[1] <- "Number of Participants"
# colnames(nap_df)[2] <- "Days of Follow-Up"
# colnames(nap_df)[3] <- "Distribution"
# 
# nap_df <- nap_df %>% mutate(Distribution = case_when(Distribution == "gamma" ~ "Gamma",
#                                                      Distribution == "norm" ~ "Normal",
#                                                      Distribution == "student" ~ "t(df=2)",
#                                                      Distribution == "mix1" ~ "Mixture1",
#                                                      Distribution == "stud3nt" ~ "t(df=3)"))
# 
# 
# nap_df$Distribution <- factor(nap_df$Distribution, levels = c("Gamma", "Normal", "Mixture1","t(df=3)","t(df=2)"))
# 
# #Only focus on sensitivity 
# #Comment out this line for results on PPV too
# # vit_df <- vit_df %>% filter(.[[1]] != "Wake PPV" & .[[1]] != "Sleep PPV")
...
# )
# 
# p + labs(color='Model',title = "Nap Sensitivity and PPV by Model, Sample Size, and RE Distribution") +
#   guides(size = F, shape = F, linetype = F)
# 
# # setwd("../Plots")
# # ggsave("NestedLoopNap.png",width = 9,height = 9)
```
